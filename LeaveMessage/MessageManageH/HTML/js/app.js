...
var _c = {
    ...
    dir: {
        app: "/MessageManageH/" + g_moudel , //注意末尾不能有/
        pc: "/MessageManageH/" + g_moudel  //注意末尾不能有/
    },
    runtime: {
        debug: true,
        fake: false,
   ...
   sever: {
      ...
      cdn: "",//cdn服务器
      host_HTML: "http://localhost:1649",///框架服务器[登陆,权限,报表]
      host_wx: "http://1ocalhost:1649" //微信服务器
  }

  sever: {
     host: "http://demo.q-mi.cn:8080",  //业务服务器
     cdn: "http://cdn.qxamoy.com/framwork/debug/2.0/res/vendor/",//cdn服务器
     host_HTML: "http://demo.q-mi.cn:8080", ///框架服务器[登陆,权限,报表]
     host_wx: "http://demo.q-mi.cn:8080" //微信服务器
 }
  
  */
    sever: {
        host: "http://127.0.0.1:8080",  //业务服务器cloud.qxamoy.com:7877
        cdn: "http://cdn.qxamoy.com/framwork/debug/2.0/res",//cdn服务器

        host_HTML: "http://cloud.qxamoy.com:7877", ///框架服务器[登陆,权限,报表]
        host_wx: "http://cloud.qxamoy.com:7877" //微信服务器
    }

};

_c.app = {
    login: _c.dir.app + "/login.html",
    regist: _c.dir.app + "/pages/regist-m.html",
    login_auto: _c.dir.app + "/pages/login-auto-m.html",
    homepage: _c.dir.app + "/index.html",
    login_third: _c.sever.host_wx + "/wx/index?state=" + window.location.origin + _c.dir.app + "/pages/login-auto-m.html?return_url=" //+ window.location.href  //"we7"
    //bind: _c.dir.app + "/pages/bind-m.html",
    //wx_login: _c.sever.host_wx + "/open/wx?return_url=" + window.location.origin + g_login_wx, //微信自动登陆
    //pay_wx: _c.sever.host_wx + "/WeChat/WeChatPay/JsApiPayPage", //微信支付回调
    //pay_result_wx: window.location.origin + "/web/app/wxpay/charge-result.html?subHTMLtem=HTMLtem", //微信授权回调
...
_c.pc = {
    login: _c.dir.pc + "/login.html",
    regist: _c.dir.pc + "/pages/regist.html",
    login_auto: _c.dir.pc + "/pages/login-auto.html",
    homepage: _c.dir.pc + "/index.html",
    login_third: "",//"we7",
    ...
    log: _c.dir.pc + "/pages/log.html"
};
_c.cm = {
    loading: _c.dir.pc + "/pages/loading.html",
    welcome: {
        enable:true,
        url: "/MessageManageH/HTML/template/views/html/invocing/home/index.html",
        title: "欢迎使用",
        width: -1,
        height: -1
    }
...
_c.shoWelcome = _c.cm.welcome.enable;
...
_c.regist = _c.isApp ? _c.app.regist : _c.pc.regist;
_c.login_auto = _c.isApp ? _c.app.login_auto : _c.pc.login_auto;
_c.homepage = _c.isApp ? _c.app.homepage : _c.pc.homepage;
_c.login_third = _c.runtime.third_login ?
    (_c.isApp ?
        _c.app.login_third : _c.pc.login_third) : "";
...
_c.user = function () {
    return { uid: _c.cookie("user_id"), name: _c.cookie("nick_name"), unit: _c.cookie("unit_name"), sub_HTMLtem: _c.cookie("sub_HTMLtem")  };
}
...
_c.random = function (preString) {
    ...
    var dateStr = year +
      ...
      //  (minute < 10 ? "0" + minute : minute) +
      //  (second < 10 ? "0" + second : second);
    return _c.checkValue(preString)+dateStr+Guid.random().split('-')[0];
}

//转换网页
_c.url = function (url, isFramWorkUrl) {
    if (_c.hasValue(isFramWorkUrl) && isFramWorkUrl === true) {//是框架网址i
        return _c.sever.host_HTML + url;
    }
...
_c.parseurl = function (url, type) {
    ...
    if (url.indexOf("@") > -1) {
        ...
        //html
        result.destUrl = _c.res("views/print/" + url.replace('*p/', "").replace('*p', "")) + "&uid=" + _c.uid() + "&unitid=" + _c.unitid();
        result.destType = "p";
    } 
    else if (url.indexOf("http") > -1) {
        //http
        result.destUrl = url;
        result.destType = "http";
        ...
        //log日志
        result.destUrl = _c.pc.log + "?id=" + url.replace("*l", "");
        result.destType = "l";
    }
    ...
    result.destUrl.replace("//", "/").replace("//", "/").replace("//", "/");
...
_c.go = function (url, waitSecond) {
    ...
    setTimeout(function () {
       ...
       //win10特殊处理
        if (_c.isWin10) {
            Win10.openUrl(url, "<i class=\"fa fa-bug  icon green\"></i>")
        }
        else {
            //防止_c.q出错
...
//获取地址栏参数
_c.q = function (name,valueIfEmpty) {
        // // 
        ...
        return _c.checkValue(data[name], valueIfEmpty);
    },
    _c.qall = function (s) {
...
_c.isRegistPage = function () {
    return _c.geturl() === _c.regist;
};
_c.isHomePage = function () {
    return _c.geturl() === _c.homepage;
};
_c.isVisitor = function () {
    return !_c.isLoginPage() && !_c.isRegistPage() && !_c.isLoadingPage() && !_c.isAutoLoginPage() &&
...
_c.pc.lib = {
    src: [
        ...
        //jquery-print
        { "jquery-print": srcurl("jquery-print/files/print.js") },
        //echarts 
        ...
        //bootstrap-treeview
        { "bootstrap-treeview": srcurl("bootstrap-treeview/bootstrap-treeview.min.js") },
        //jquery.cookie
...
_c.tool = function () {
    ...
    $.extend({
        ...
        Ajax: function (cfg) {
            ...
            cfg.url = cfg.url.replace(/undefined/g, ""); //替换undefined
        ...
        dealError: function (errorSrc, errorMessage, stack) {
            _c.warn(stack);
            $.alert("出错啦~<hr/><p style='color:red;'>" + errorMessage + "</p>", 5, function () {
                try {
                    //iframe窗
                    layer.open({
                        type: 1,
        ...
        closeWindow: function (msg, seconds) {
            $(".btn").remove();
            if (_c.hasValue(msg)) {
                $.msg(msg + "<br/><span id='lb_time'>" + seconds + "</span>秒后自动关闭窗口", 1);
            }
               
            setTimeout(function() {
                var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引
                parent.layer.close(index); //再执行关闭   
            }, _c.checkValue(seconds*1000,2200));
},
        dealAjax: function (data, code, msg, url) { //回调函数,result,返回值 
            ...
            case 9:
                { //下载文件
                    ...
                    _c.go(_c.url(data[data.name]));
                   /* $.ajax({
                        url: _c.url(data[data.name]),
                        success: function () {
                            debugger
                            _c.go(_c.url(data[data.name]));
                            //window.open(_c.url(data[data.name]));    
            ...
            case 13:
                    {
                        $.msg(msg, 1);
                        _c.go(_c.parseurl(url).destUrl,1);
                };
                break;
            case 14:
                {
        ...
        vals_m: function (id, type) {
            ...
            var domsFromId = $("input[id*='-'],textarea[id*='-'],select[id*='-']"); //只住区input和area
            var doms = _c.union(domsFromName, domsFromId, "outerHTML");
        ...
        submitPage: function (url, doAfterSuccess, doBeforeSubmit) {
            ...
            var jsonPageData = $("#page_data").val();//原来的-依赖于bindPage
            var submitData = {};
            //  
        ...
        bindPrint: function (url, tplArray, doBeforBind) {
            $.bindPage(url, tplArray, doBeforBind,
                function () {
                    Print("body");
                    $("body").click(function () {
                        _c.go(-1);
        ...
        bindPage: function (dataUrlArray, tplArray, doBeforeBinded, doAfterBinded, isReplace) {
            //isReplace是否覆盖内容，打印也专用 2018-04-20
            isReplace = _c.checkValue(isReplace, false);
            //检查页面是否规范2017-12-24
            $.checkPage();
            // 
            $.Ajaxs(dataUrlArray, function (data) {
                // 
                //缓存最原始的数据
                $("body").append("<input type='hidden' id='page_data' value='" + _c.toJson(data) + "'>");
...
_c.bll = function () {
    ...
    if (_c.isAutoLoginPage()) {
        ...
        $.loading();
        $.msg("加载中...");
        autoLogin(_c.q("userId"), _c.q("roleString"), _c.q("subHTML"), _c.q("site"), _c.q("userName"));
        return;
    }
    //处理登陆
    if (_c.isLoginPage()) {
...
window.onload = function () {
    ...
    //处理访客
        ...
        if (_c.hasValue(_c.login_third)) {
            if (_c.login_third == "we7") {
                _c.cleanPage();
                _c.log("页面会话超时，请关闭页面后重新进入！");
            } else {
                _c.log("加载中...");
                //debugger    
                _c.go(_c.login_third );
            }
    ...
    $.Ajax({
        ...
        success: function (data, code) {
            if (code === 1) {
                ...
                _c.cookie('sub_HTMLtem', data.sub_HTMLtem);//存储子系统
...
function autoLogin(userId, roleString, subHTML, site, userName) {
    ...
    var index = $.loading();
    $.ajax({
        url: _c.url("/Open/autoLogin?userId=" + userId + "&roleString=" + roleString + "&subHTML=" + subHTML + "&site=" + site + "&userName=" + userName, true),
        success: function (result) {
            $.loaded(index);
            var code = result.code;
            var data = _c.toObject(result.jsonData);
